    ...
    function strFuncBuilder(method: "contains" | "startswith" | "endswith") {
        return function (s: StringFilterValue, opt?: StringOptions): IExpression {
            if (opt?.caseInsensitive) {
                return makeExp(
                    `${method}(tolower(${key}), ${typeof s == "string" ? `'${s.toLocaleLowerCase()}'` : `tolower(${(s as FilterProperty)._key})`})`,
                );
...
export function createFilter(descriptor: QueryDescriptor) {
    return (
        keyOrExp: string | ((filter: RecursiveFilterProxy) => IExpression),
        exp?: (builder: FilterMethods) => IExpression,
    ) => {
        const expr =
            typeof keyOrExp === "string" && exp
                ? exp(filterBuilder(keyOrExp))
                : (keyOrExp as (filter: RecursiveFilterProxy) => IExpression)(makeFilter());
